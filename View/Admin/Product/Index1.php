...
<form class="d-flex me-3" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="get">
    ...
    if ($conn) {
        ...
        if (isset($_GET['search']) && !empty($_GET['search'])) {
            ...
            $search_condition = " WHERE TenSanPham LIKE '%$search_term%'";
        }

        // Câu truy vấn lấy dữ liệu với phân trang và điều kiện tìm kiếm
        $sql = "SELECT * FROM sanpham $search_condition ORDER BY MaLoai ASC LIMIT $position, $display";
        $SP = $conn->query($sql);
        ...
        if ($SP) {
            ...
            if ($SP->rowCount() > 0) {
                ...
                <div class="d-flex flex-wrap justify-content-center">
                    <?php foreach ($SP as $kh): ?>
                    	<tr>
                                <td><?php echo $kh['MaSanPham'];?></td>
                                <td><a href="Detail.php?id=<?php echo $kh['MaSanPham']?>" class="fw-bold"><?php echo $kh['TenSanPham'];?></a></td>
                                <td><?php echo $kh['Gia'];?></td>
                                <td><?php echo $kh['TinhTrang'];?></td>
                                <td>
                                    <?php 
                                    $maloai= $kh['MaLoai'];
                                    $loai = "SELECT tenloai FROM loaisanpham WHERE maloai=$maloai";
                                    $stmt_loai = $conn->prepare($loai);
                                    $stmt_loai->execute();
                                    $row = $stmt_loai->fetch(PDO::FETCH_ASSOC);
                                    echo $row['tenloai'];
                                    ?>
                                </td>
                                <td><?php echo $kh['MoTa'];?></td>
                                <td><?php echo $kh['URLHinhAnh'];?></td>
                                <td>
                                    <a class="btn btn-success text-white" href="Edit.php?id=<?php echo $kh['MaSanPham']?>">Sửa</a>
                                    <a class="btn btn-danger text-white" href="Delete.php?id=<?php echo $kh['MaSanPham']?>">Xóa</a>
                                </td>
                            </tr>
                    <?php endforeach; ?>
                </div>
                ...
                $sql_count = "SELECT COUNT(*) FROM sanpham $search_condition";
